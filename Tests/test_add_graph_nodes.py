...
class TestAddGraphNodes(unittest.TestCase):
    ...
    @patch('add_graph_nodes.Neo4jBase.__init__')
    @patch('add_graph_nodes.GraphDatabase.driver')
    ...
    @patch('add_graph_nodes.pd.read_csv')
    ...
    @patch('add_graph_nodes.os.remove')
    @patch('add_graph_nodes.pd.DataFrame.to_csv')
    def test_process_and_add_nodes_from_directory(
        ...
        mock_os_remove.assert_called_once_with(os.path.join('dummy_directory', 'combined_temp.csv'))
